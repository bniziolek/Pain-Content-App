...
function Sidebar({ className, onNavigate }: SidebarProps) {
  ...
  const allClinicianLinks = [
    ...
    },
    {
      ...
      icon: Route,
      tourId: "nav-pathways",
      flagKey: "pathways_enabled",
    },
    {
      href: "/follow-ups",
      label: "Follow-ups",
      icon: Bell,
      tourId: "nav-followups",
      flagKey: "follow_ups_enabled",
    },
    {
      href: "/history",
      label: "History",
      ...
      {!isAdmin && user?.subscriptionTier === "basic" && (
        <div className="mx-4 mb-4 p-3 rounded-lg bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200">
          ...
          <p className="text-xs text-amber-700 mb-3">
            Unlock Care Pathways, Email Delivery & more
          </p>
        ...
        {bottomLinks.slice(0, 5).map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href}>
              ...
              >
                <Icon
                  className={cn("mb-0.5", isActive ? "w-6 h-6" : "w-5 h-5")}
                />
                <span
                  className={cn(
                    "font-medium",
        ...
        <Sheet open={isMobileOpen} onOpenChange={setIsMobileOpen}>
          <SheetTrigger asChild>
            <Button
              variant="outline"
              size="icon"
              className="min-w-[48px] min-h-[48px] shadow-md bg-background/95 backdrop-blur-sm border-2"
